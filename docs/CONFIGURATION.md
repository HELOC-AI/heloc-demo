...
> 适用范围：`heloc-demo`（本仓库）与 `heloc-email-service`。
...
- **Railway 引用变量**（`${{service.VAR}}`）让调用方直接引用被调用方的 key 和域名：值只维护一处，轮换时只改被调用方，调用方重新部署即拿到新值。
  email-service 虽然是独立仓库，但部署在**同一个 Railway Project** 里，因此同样可被引用。
...
- `.env.example` 与 schema 保持一致（CI 中加一个简单脚本校验 key 集合相同）。
...
pnpm --filter @heloc/intake dev                # tsx watch --env-file=.env src/main.ts
```
